...
Shader "Toony Colors Pro 2/User/Sketch Shader Z Cutout"
{
	Properties
	{
	...
	SubShader
	{
		...
		#pragma shader_feature_local _ _ALPHAPREMULTIPLY_ON
			...
			) : SV_Target
			{
				...
				// World-space Z clipping
				float zClip = (_InvertZClip > 0.5)
					? (positionWS.z - _ZClipPosition)
					: (_ZClipPosition - positionWS.z);
				clip(zClip);
		...
		HLSLINCLUDE
			...
			) : SV_TARGET
			{
				...
				// World-space Z clipping
				float zClip = (_InvertZClip > 0.5)
					? (input.positionWS.z - _ZClipPosition)
					: (_ZClipPosition - input.positionWS.z);
				clip(zClip);
