	...
	) : SV_Target
{
	...
	// Render Texture Overlay – alpha-blended on top of the fully-lit surface colour.
	#if !defined(SHADOW_CASTER_PASS) && !defined(DEPTH_NORMALS_PASS) && !defined(UNITY_PASS_META)
	{
		float2 overlayUV = input.texcoords.xy * _OverlayMap_ST.xy + _OverlayMap_ST.zw;
		overlayUV.x = _OverlayFlipX > 0.5 ? 1.0 - overlayUV.x : overlayUV.x;
		overlayUV.y = _OverlayFlipY > 0.5 ? 1.0 - overlayUV.y : overlayUV.y;
		half4 overlaySample = tex2D(_OverlayMap, overlayUV);
		half overlayBlend = overlaySample.a * _OverlayOpacity;
		color.rgb = lerp(color.rgb, overlaySample.rgb, overlayBlend);
	}
	#endif
