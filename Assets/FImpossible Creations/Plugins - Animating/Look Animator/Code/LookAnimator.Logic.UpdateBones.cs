...
namespace FIMSpace.FLook
{
    ...
    public partial class FLookAnimator
    {
        ...
        Vector3 lastPerfectLookTarget = Vector3.zero;
        float perfectWasClampAt = 0f;
        private void PostAnimatingTweaks()
        {
            if (OverrideHeadForPerfectLookDirection > 0f)
            {
                ...
                Vector3 targetLookDir = activeLookPosition - headOv.transform.position;
                float smoothingSpeed = 14f;

                if (FollowMode != EFFollowMode.FollowJustPosition)
                {
                    if (activeLookTarget == null || (Time.time - TargetChangedAt < 1f) )
                        lastPerfectLookTarget = Vector3.Lerp(lastPerfectLookTarget, targetLookDir, delta * smoothingSpeed);
                    else
                        lastPerfectLookTarget = targetLookDir;
                }
                else
                {
                    lastPerfectLookTarget = targetLookDir;
                ...
                if ( ApplyClampingForOverrideMode)
                {
                    ...
                    {
                        if (perfectWasClampAt == 1) { } // To Implement -> Smooth out direction change
                        perfectWasClampAt = -1f;
                        angles.y = GetClampedAngle(angles.y, XRotationLimits.x, XElasticRange, -1f);
                    }
                    else if (angles.y > XRotationLimits.y) // Looking to the right clamp angle
                    {
                        if (perfectWasClampAt == -1) { } // To Implement -> Smooth out direction change
                        perfectWasClampAt = 1f;
                        angles.y = GetClampedAngle(angles.y, XRotationLimits.y, XElasticRange);
                    }
                    else perfectWasClampAt = 0f;
                ...
                LookBones[0].Transform.rotation = Quaternion.Lerp(preRt, headOv.transform.rotation, OverrideHeadForPerfectLookDirection * LookAnimatorAmount);
            }
        }
