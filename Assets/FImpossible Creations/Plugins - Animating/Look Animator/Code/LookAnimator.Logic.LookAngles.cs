...
namespace FIMSpace.FLook
{
    ...
    public partial class FLookAnimator
    {
        ...
        public Vector2 LimitAnglesCalculations(Vector2 angles)
        {
            ...
            {
                float range = StopLookingAbove * StopLookingAboveFactor;
                range -= StopLookAboveMargin;

                if (Mathf.Abs(angles.y) <= range)
                {
                    LookState = EFHeadLookState.Null;
                }
                else
                {
                    // -> Still clamp to avoid reaching unclamped angle
