        ...
        >
    {
        ...
        protected override void Update()
        {
            ...
            if (IsInitialized)
            {
                ...
                var singletonExists = !ReferenceEquals(singleton, null) && (bool)singleton;
                var clientActive = singletonExists && singleton.IsClient && singleton.IsConnectedClient;
                var serverActive = singletonExists && singleton.IsServer;
                var networkActive = singletonExists && singleton.isActiveAndEnabled && (clientActive || serverActive);

                if (networkActive)
                {
                    // Check what mode the network is in
                    var server = singleton.IsServer;
                    var client = singleton.IsClient;
