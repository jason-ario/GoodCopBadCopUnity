Shader "Unlit/BuiltinHalfToneMonoShader"
{
    Properties
    {
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            fixed4 frag (v2f i, UNITY_VPOS_TYPE screenPos : VPOS) : SV_Target
            {
                ...
                float grayRate = dot(col.rgb, float3(0.299, 0.587, 0.114));
                grayRate = saturate((grayRate-_FillRate*2)*_Contrast+1);

                fixed4 dotCol = tex2D(_DotTex, base_uv)*_Color*(1-grayRate);
                float dotAlpha = saturate((dotCol.a - grayRate)*10);
